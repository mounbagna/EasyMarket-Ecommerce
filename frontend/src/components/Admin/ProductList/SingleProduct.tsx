...
const SingleProduct = () => {
  ...
  const deleteProduct = async(id:number)=>{
     ...
     try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/products/${id}`,{method:"DELETE"});
  ...
  return (
    ...
    <div key={product.id} className="flex items-center border-t border-gray-3 py-5 px-7.5">
      <div className="min-w-[400px]">
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            <div className="flex items-center justify-center rounded-[5px] bg-gray-2 max-w-[80px] w-full h-17.5">
              <img width={200} height={200} 
              src={getImageUrl(product.preview[0])} alt="shop owner" />
            </div>
            <div>
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">{product.quantity}</p>
      </div>

      <div className="min-w-[200px]">
        <p className="text-dark">{product.shopowner_id}</p>
      </div>

      <div className="min-w-[200px]">
        <p className="text-dark">{product.location}</p>
      </div>

      <div className="min-w-[200px]">
        <p className="text-dark">{product.condition_type}</p>
      </div>
