...
const SingleGridItem = ({ item,showOwnerAction = true }: Props) => {
  const {user} = useAuth();
  const { openModal } = useModalContext();
  const { openEditItemModal } = useEditItemModalContext();
  ...
  const isOwner = user && user.id === item.shopowner_id;
    ...
    <div className="group">
      ...
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-2">
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          {showOwnerAction && isOwner && (
            <div className="flex gap-2 mt-3">
              <button
               onClick={()=>{
                openEditItemModal(item)
              }}
               className="flex item-center gap-1 px-3 py-2 bg-blue text-white rounded hover:bg-blue">
                <Pencil size={18} />
                </button>
               
                <button
                  onClick={()=>handleDelete(item.id)}
                  className="flex item-center gap-1 px-3 py-2 bg-red text-white rounded hover:bg-red">
                    <Trash2 size={18} />
                </button>
              </div>
            )}
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-dark">{item.discounted_price} FCFA</span>
        <span className="text-dark-4 line-through">{item.price} FCFA</span>
      </span>
      <div className="flex items-center gap-1 text-xs font-medium">
        {item.condition_type === "second_hand" ? (
          <>
          <span className="w-2.5 h-2.5 bg-yellow rounded-full"></span>
          <span className="text-dark">Second Hand</span>
          </>
        ) : (
          <>
