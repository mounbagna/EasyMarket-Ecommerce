...
import {normalizeArray, getImageUrl} from "@/utils/normalizedUtils"
...
const QuickViewModal = () => {
  ...
  const thumbnails = normalizeArray(product?.thumbnail) 
                ...
                <div className="flex flex-col gap-5">
                  {thumbnails.slice(0,2).map((img, index) => (
                    <button
                      ...
                      key={index}
                      className={`flex items-center justify-center w-20 h-20 overflow-hidden rounded-lg bg-gray-1 ease-out duration-200 hover:border-2 hover:border-blue ${activePreview === index && "border-2 border-blue"
                        }`}
                    >
                      <img
                        src={getImageUrl(img)}
